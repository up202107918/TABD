...
Generated for the report and oral presentation. Regenerate **after ETL full load** (both `aut_2017` and `aut_2021` recommended) — see [README.md](../../README.md) and [reproducibility.md](../reproducibility.md).

## Recommended (one command)

From repository root, same PowerShell session as ETL (`DB_*` set):
...
After a full ETL reload or `seat_result` load, re-run so `demo_results.txt` reflects current data.
